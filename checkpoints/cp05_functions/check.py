...
Imports the three functions from main.py (draw_dashboard, format_distance,
handle_controls) and exercises them directly.
...
def main():
    ...
    needed = ("draw_dashboard", "format_distance", "handle_controls")
    for fn in needed:
    ...
    # --- format_distance -------------------------------------------------------
    try:
        result = student.format_distance(340)
        check("format_distance(340) returns \"340 m\"", result == "340 m",
              f"got {result!r}")
    except Exception as exc:
        check("format_distance(340) returns \"340 m\"", False, repr(exc))

    try:
        result = student.format_distance(1500)
        check("format_distance(1500) returns \"1.5 km\"", result == "1.5 km",
              f"got {result!r}")
    except Exception as exc:
    ...
    drifted_line = next((c for c in text_calls if c["text"].startswith("DRIFTED")), None)
    check("draw_dashboard draws the DRIFTED line correctly (text, position, size, color)",
          drifted_line == {"text": "DRIFTED: 340 m", "pos": (engine.WIDTH - 16, engine.HEIGHT - 26),
                            "size": 13, "color": (120, 140, 155), "anchor": "topright"},
          f"got {drifted_line}")

    check("draw_dashboard draws exactly 4 lines of HUD text (no extras, none missing)",
          len(text_calls) == 4, f"got {len(text_calls)}: {[c['text'] for c in text_calls]}")
