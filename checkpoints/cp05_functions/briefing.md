...
Open it side by side with this one. The sideways movement flags and the
odometer readout are the pieces that are genuinely new - and the odometer
readout needs a third function, `format_distance`, to decide how to show
it.
...
Open `main.py`. Write THREE functions between `BEGIN YOUR CODE (Checkpoint
...
### 2. `draw_dashboard(screen, sub, alert)` - void
...
Then add a fourth line, using the function you just wrote: call
`format_distance(sub.total_drift)` and draw the result as
...
void instead of value-returning. `format_distance` is the opposite: it
doesn't draw or change anything, it just hands a string back to whoever
called it. Inside `draw_dashboard`, you're calling `format_distance` -
one of your own functions calling another - plus `hull_status`,
`oxygen_state`, and `can_descend`, all from Checkpoint 3. A void function
...
all three functions exist will crash with a `NameError` - that's expected,
...
flip from meters to kilometers once it passes 1000.

---

## Done when

`python check.py` prints **18 / 18** (100 points). It checks:
...
- `draw_dashboard()` draws exactly the right four lines of text plus the
...
- Go copy the lines from your own Checkpoint 4 `frame()` first, then figure
  ...
  `handle_controls` from memory - `format_distance` is the one function
  here with no source to copy from.
...
- If `check.py` says `draw_dashboard` drew the wrong number of lines, count
  your `draw_hud_text` calls against the five things it's supposed to draw
  (hull status, O2, STATUS, the hint line, and DRIFTED).
...
the `SETUP.md` setup if you haven't, then write the three functions here,
